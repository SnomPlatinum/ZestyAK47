...
func add_segment(pos):
	...
	$GameArea.add_child(SnakeSegment)
...
func check_out_of_bounds():
	if snake_data[0].x < -9 or snake_data[0].x > 9 or snake_data[0].y < -9 or snake_data[0].y > 9:
		end_game()
...
func move_food():
	while regen_food:
		regen_food = false
		food_pos = Vector2(randi_range(-9, 9), randi_range(-9, 9))
		for i in snake_data:
			if food_pos == i:
				regen_food = true
	$GameArea/Food.position = (food_pos * cell_size)
	regen_food = true
